...
RAW_CONFIG="${RAW_CONFIG:-$PART3_ROOT/configs/project_pretrained_raw.json}"
CONF_CONFIG="${CONF_CONFIG:-$PART3_ROOT/configs/project_pretrained_conf.json}"
FULL_CONFIG="${FULL_CONFIG:-$PART3_ROOT/configs/project_pretrained_full.json}"
...
WAYMO_DYNAMICRAFTER_RESOLUTION="${WAYMO_DYNAMICRAFTER_RESOLUTION:-320_512}"
SOURCE_WORKSPACE_ROOT="${SOURCE_WORKSPACE_ROOT:-$PART3_ROOT/workspace}"
OUTPUT_WORKSPACE_ROOT="${OUTPUT_WORKSPACE_ROOT:-$(python3 -c "import json, sys; data=json.load(open(sys.argv[1])); print(data.get('workspace_root', sys.argv[2]))" "$FULL_CONFIG" "$PART3_ROOT/workspace_pretrained")}"
...
usage() {
  cat <<EOF
Build Part 3 pretrained raw/conf/full ablation scenes by reusing one shared pseudo-view generation per dataset.
...
Run all three datasets:
  SCENES="405841_FRONT DL3DV-2 Re10k-1" bash part3/scripts/run_reuse_pseudo_pretrained_ablation.sh
...
Defaults:
  ...
  WAYMO_DYNAMICRAFTER_RESOLUTION=$WAYMO_DYNAMICRAFTER_RESOLUTION
  SOURCE_RUN_ID_TEMPLATE=$SOURCE_RUN_ID_TEMPLATE
  RUN_PREFIX_TEMPLATE=$RUN_PREFIX_TEMPLATE
  SOURCE_WORKSPACE_ROOT=$SOURCE_WORKSPACE_ROOT
  OUTPUT_WORKSPACE_ROOT=$OUTPUT_WORKSPACE_ROOT

Variant outputs for each scene:
  $OUTPUT_WORKSPACE_ROOT/runs/<scene>/<scene>_pretrain_raw
  $OUTPUT_WORKSPACE_ROOT/runs/<scene>/<scene>_pretrain_conf
  $OUTPUT_WORKSPACE_ROOT/runs/<scene>/<scene>_pretrain_full
  $OUTPUT_WORKSPACE_ROOT/hybrid_scenes/<scene>_pretrain_raw
  $OUTPUT_WORKSPACE_ROOT/hybrid_scenes/<scene>_pretrain_conf
  $OUTPUT_WORKSPACE_ROOT/hybrid_scenes/<scene>_pretrain_full

Useful overrides:
  SCENE=DL3DV-2 bash part3/scripts/run_reuse_pseudo_pretrained_ablation.sh
  SCENES="405841_FRONT DL3DV-2 Re10k-1" REBUILD_VARIANTS=1 bash part3/scripts/run_reuse_pseudo_pretrained_ablation.sh
  VARIANTS="conf full" bash part3/scripts/run_reuse_pseudo_pretrained_ablation.sh
...
Notes:
  ...
  raw keeps the pretrained manifest shape but disables MASt3R/SEA-RAFT component use.
  REBUILD_VARIANTS=1 only removes per-variant pretrained_confidence/ and hybrid_scenes/<variant> under OUTPUT_WORKSPACE_ROOT.
...
resolution_for_scene() {
  case "$1" in
    405841*|waymo*|Waymo*)
      echo "$WAYMO_DYNAMICRAFTER_RESOLUTION"
      ;;
    *)
      echo "$DYNAMICRAFTER_RESOLUTION"
      ;;
  esac
}

resolution_tag_for_scene() {
  local scene_resolution
  scene_resolution="$(resolution_for_scene "$1")"
  printf '%s' "$scene_resolution" | tr '_' 'x'
}

source_run_id_for_scene() {
  local scene="$1"
  local resolution_tag
  resolution_tag="$(resolution_tag_for_scene "$scene")"
  if [ -n "$SOURCE_RUN_ID" ]; then
    echo "$SOURCE_RUN_ID"
  else
    format_template "$SOURCE_RUN_ID_TEMPLATE" "$scene" "$resolution_tag"
  fi
}
...
run_prefix_for_scene() {
  ...
  if [ -n "$RUN_PREFIX" ]; then
    ...
    format_template "$RUN_PREFIX_TEMPLATE" "$scene" "$resolution_tag"
  fi
}
...
pretrained_confidence_manifest_is_current() {
  ...
  python3 -c \
    "import json, sys; data=json.load(open(sys.argv[1])); formula=data.get('confidence_formula') or {}; feature=data.get('feature') or {}; temporal=data.get('temporal') or {}; ok=(data.get('route') == 'pretrained' and formula.get('type') == 'hard_validity_times_soft_floor' and int(formula.get('version', 0)) >= 3 and feature.get('backend') == 'mast3r' and temporal.get('backend') == 'sea_raft'); sys.exit(0 if ok else 1)" \
    "$confidence_manifest"
}
...
build_variant() {
  ...
  local variant="$5"
  local run_id="${run_prefix}_pretrain_${variant}"
  local config
  config="$(variant_config "$variant")"
  local target_run_dir="$OUTPUT_WORKSPACE_ROOT/runs/$scene/$run_id"
  ...
  local confidence_manifest="$target_run_dir/pretrained_confidence/confidence_manifest.json"
  local hybrid_scene_dir="$OUTPUT_WORKSPACE_ROOT/hybrid_scenes/$run_id"
  local hybrid_manifest="$hybrid_scene_dir/part3_hybrid_manifest.json"

  echo
  echo "==> Building pretrained variant: $run_id"
  echo "==> Config: $config"
  ...
  if ! pretrained_confidence_manifest_is_current "$confidence_manifest"; then
    echo "==> Existing pretrained confidence manifest is stale; rebuilding confidence/hybrid"
    rebuild_this_variant=1
  fi

  if [ "$rebuild_this_variant" = "1" ]; then
    echo "==> Removing per-variant pretrained confidence and hybrid scene only"
    rm -rf "$target_run_dir/pretrained_confidence" "$hybrid_scene_dir"
  fi
  ...
  run_if_missing "$confidence_manifest" "$BUILD_ENV" \
    python3 "$PART3_ROOT/apps/generate_confidence_pretrained.py" \
      --config "$config" \
      --pseudo-manifest "$target_pseudo"

  run_if_missing "$hybrid_manifest" "$BUILD_ENV" \
    python3 "$PART3_ROOT/apps/build_hybrid_pretrained.py" \
      --config "$config" \
  ...
  echo
  echo "==> Pretrained variant ready: $run_id"
...
build_scene() {
  local scene="$1"
  local scene_resolution
  scene_resolution="$(resolution_for_scene "$scene")"
  local source_run_id
  source_run_id="$(source_run_id_for_scene "$scene")"
  local run_prefix
  run_prefix="$(run_prefix_for_scene "$scene")"
  local source_run_dir="$SOURCE_WORKSPACE_ROOT/runs/$scene/$source_run_id"
  ...
  echo "==> DynamiCrafter resolution: $scene_resolution"
  ...
  assert_source_pseudo_resolution "$source_pseudo" "$scene_resolution"
...
echo "==> Selected pretrained variants: $VARIANTS"
echo "==> Default DynamiCrafter resolution: $DYNAMICRAFTER_RESOLUTION"
echo "==> 405841/Waymo DynamiCrafter resolution: $WAYMO_DYNAMICRAFTER_RESOLUTION"
echo "==> Source workspace root: $SOURCE_WORKSPACE_ROOT"
echo "==> Output workspace root: $OUTPUT_WORKSPACE_ROOT"
...
echo
echo "==> Reusable pretrained pseudo-view ablation build complete."
